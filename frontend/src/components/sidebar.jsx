      ...
      <div className="flex-grow overflow-y-auto space-y-4">
        {filteredHistory.length ? (
          filteredHistory.map(
            (
              chat,
              index // Add index as a second argument to map
            ) => (
              <div key={chat.id}>
                ...
                <ul className="space-y-2">
                  <li
                    className={`p-1 rounded-lg relative cursor-pointer flex justify-between items-center ${
                      index === 0 ? "bg-gray-100" : "" // Apply gray background only to the first chat
                    } ${
                      chat.active || hoveredChat === chat.id
                        ? "bg-gray-100"
                        : ""
                  ...
                  >
                    {/* Chat Message */}
                    <span className="block max-w-[98%] overflow-hidden whitespace-nowrap relative">
                      {chat.message.charAt(0).toUpperCase() +
                        chat.message.slice(1)}
                      <span
                        className={`absolute inset-y-0 right-0 w-2 ${
                          chat.message.length > 30 ? "bg-gradient-to-l from-white" : ""
                        } `}
                      ></span>
                    </span>
